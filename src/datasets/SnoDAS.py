...
root_folder = getRootFolder(
    dataset_name=dataset_name, fallback_name="HGS"
)  # get dataset root folder based on environment variables
...
geotransform = (
    -130.516666666667 - 0.00416666666666052,
    0.00833333333333333,
    0,
    24.1000000000000 - 0.00416666666666052,
    0,
    0.00833333333333333,
)
size = (8192, 4096)  # (x,y) map size of SnoDAS grid
# make GridDefinition instance
SnoDAS_grid = GridDefinition(
    name=dataset_name, projection=None, geotransform=geotransform, size=size
)
...
binary_varatts = dict(  # forcing variables (offset_code='P001', downscale_code='S', type_code='v0')
    liqprec=dict(
        ...
        v_code="lL00",
        t_code="T0024",
        description="24 hour total, 06:00 UTC-06:00 UTC",
        variable_type="Driving",
    ...
    Tsnow=dict(
        ...
        description="",
        variable_type="State",
        long_name="Snow Pack Average Temperature",
    ),
    ...
    snwmlt=dict(
        ...
        v_code="bS__",
        t_code="T0024",
        description="Total of 24 per hour melt rates, 06:00 UTC-06:00 UTC",
        variable_type="diagnostic",
        long_name="Snow Melt Runoff at the Base of the Snow Pack",
    ),
    evap_snow=dict(
        name="evap_snow",
        units="m/day",
        ...
        v_code="lL00",
        t_code="T0024",
        description="Total of 24 per hour sublimation rates, 06:00 UTC-06:00 UTC",
        variable_type="diagnostic",
        long_name="Sublimation from the Snow Pack",
    ),
    evap_blow=dict(
        name="evap_blow",
        units="m/day",
        ...
        description="Total of 24 per hour sublimation rates, 06:00 UTC-06:00 UTC",
        variable_type="diagnostic",
        long_name="Sublimation of Blowing Snow",
    ),
)
...
netcdf_varatts = dict(  # forcing variables
    ...
    snowh=dict(name="snowh", units="m", scalefactor=1.0, long_name="Snow Height/Depth"),
    Tsnow=dict(
        name="Tsnow",
        units="K",
        scalefactor=1.0,
    ...
    time_stamp=dict(
        name="time_stamp", units="", long_name="Time Stamp"
    ),  # readable time stamp (string)
...
netcdf_varlist = [
    varname
    for varname in list(netcdf_varatts.keys())
    if varname not in ("time", "time_stamp", "lat", "lon")
]
...
netcdf_filename = f"{dataset_name.lower()}_{{VAR:s}}_daily.nc"
...
def getFilenameFolder(varname=None, date=None, root_folder=root_folder, lgzip=True):
    ...
    # construct filename
    if varatts["variable_type"].lower() == "driving":
        var_code = "v0" + varatts["product_code"] + "S"
    else:
        var_code = "v1" + varatts["product_code"]
    t_code = varatts["t_code"]
    if varatts["product_code"] == "1044" and date < dt.datetime(2010, 2, 17):
        t_code = "T0001"
    ...
    filename = "zz_ssm{:s}{:s}TTNATS{:s}05{:s}{:s}.dat".format(
        var_code, agg_code, datestr, I_code, offset_code
    )
    if lgzip:
        filename += ".gz"
    # contruct folder
    mon = "{:02d}_{:s}".format(date.month, name_of_month[date.month - 1][:3].title())
    folder = "{:s}/data/{:04d}/{:s}/SNODAS_unmasked_{:s}/".format(
        root_folder, date.year, mon, datestr
    )
...
def readBinaryData(fobj=None, lstr=True):
    ...
    if lstr:
        ...
        data = np.fromstring(
            fobj.read(), dtype=binary_dtype, count=-1
        )  # read binary data
    else:
...
def readBinaryFile(
    ...
    # find file
    folder, filename = getFilenameFolder(
        varname=varname, date=date, root_folder=root_folder, lgzip=lgzip
    )
    filepath = folder + filename
    # check if present
    try:
        # open file (gzipped or not)
        with gzip.open(filepath, mode="rb") if lgzip else open(
            filepath, mode="rb"
        ) as fobj:
            data = readBinaryData(
                fobj=fobj,
        ...
        if scalefactor is None:
            scalefactor = (
                binary_varatts[varname]["scalefactor"]
                * netcdf_varatts[varname]["scalefactor"]
            )
        if scalefactor != 1:
            fdata *= scalefactor
    except IOError:
        if lmissing:
            print(
                (
                    "Warning: data for '{}' missing - creating empty array!\n  ('{:s}')".format(
                        date, folder
                    )
                )
            )
            ...
            if lmask:
                fdata = np.ma.masked_all(snodas_shape2d, dtype=netcdf_dtype)
            else:
                fdata = np.zeros(snodas_shape2d, dtype=netcdf_dtype) + missing_value
        else:
            print(("Point of failure: {}/{}".format(varname, date)))
            raise IOError(
                "Data folder for '{}' is missing:\n  '{:s}'".format(date, folder)
            )
    except DataError:
        if lmissing:
            print(
                (
                    "Warning: data for '{}' incomplete - creating empty array!\n  ('{:s}')".format(
                        date, folder
                    )
                )
            )
...
def creatNetCDF(
    ...
    if varatts is None:
        varatts = netcdf_varatts
    if ncatts is None:
        ncatts = netcdf_settings
    # create Dataset/file
    ...
    if "scalefactor" in atts:
        del atts["scalefactor"]
    add_var(
        ds,
        atts["name"],
        dims=axes,
...
def getGeoCoords(xvar, x_coords=None, y_coords=None):
    ...
    if x_coords is None:
        x_coords = x_coords_def
    if y_coords is None:
        y_coords = y_coords_def
...
def checkGeoReference(xds, geoargs=None, grid=None):
    ...
    if geoargs is None:
        ...
        if "proj4" in xds.attrs:
            ...
            if grid:
                print(
                    "No projection information available for selected grid '{}'; assuming geographic (lat/lon).".format(
                        grid
                    )
                )
            xds = addGeoReference(
...
def loadSnoDAS_Daily(
    ...
    **kwargs,
    ...
    if lgeospatial:
        # only some variables available with that bias correction
        default_varlist = bc_varlists.get(bias_correction, netcdf_varlist)
        if grid is None:
            grid = "snodas"
        xds = loadXRDataset(
            varname=varname,
            ...
            shape=shape,
            station=station,
            default_varlist=default_varlist,
        ...
        warn(
            "DEPRECATION WARNING: this is only for backwards compatibility and should not be used; use lgeospatial=True instead."
        )
        if time_chunks:
            chunks = dict(
                time=time_chunks
            )  # shorthand - only for backwards compatibility
        folder = daily_folder
        if grid:
            folder = "{}/{}".format(
                folder, grid
            )  # non-native grids are stored in sub-folders
        # load variables
        if bias_correction is None and "resolution" in kwargs:
            bias_correction = kwargs["resolution"]  # allow backdoor
        if varname and varlist:
            ...
            if grid:
                varname = "{}_{}".format(
                    varname, grid
                )  # also append non-native grid name to varname
            if bias_correction:
                varname = "{}_{}".format(
                    bias_correction, varname
                )  # prepend bias correction method
            filepath = "{}/{}".format(
                folder, netcdf_filename.format(VAR=varname).lower()
            )
            ...
            if bias_correction:  # prepend bias correction method to varnames
                varlist = [
                    "{}_{}".format(bias_correction, varname) for varname in varlist
                ]
            # load multifile dataset (variables are in different files
            filepaths = [
                "{}/{}".format(folder, netcdf_filename.format(VAR=varname).lower())
                for varname in varlist
            ]
            # xds = xr.open_mfdataset(filepaths, chunks=chunks, **kwargs) # tends to load everything into memory... not good...
            xds = xr.merge(
                [xr.open_dataset(fp, chunks=chunks, **kwargs) for fp in filepaths]
            )
        # add projection
        if lgeoref:
            xds = checkGeoReference(xds, geoargs=geoargs, grid=grid)
    # add some labels... and return dataset
    xds.attrs["name"] = "SnoDAS"
    xds.attrs["title"] = xds.attrs["name"] + " Daily Timeseries"
    if bias_correction is not None and "bias_correction" not in xds.attrs:
        xds.attrs["bias_correction"] = bias_correction
    if resampling is not None and "resampling" not in xds.attrs:
        xds.attrs["resampling"] = resampling
    return xds
    ...
    **kwargs,
    ...
    if lxarray:
        ...
        if varname and varlist:
            ...
            # load a single variable
            xds = xr.open_dataset(
                folder + tsfile.format(bc_str, varname, grid_str),
                chunks=chunks,
                **kwargs,
            )
        else:
            ...
            # load multifile dataset (variables are in different files
            filepaths = [
                folder + tsfile.format(bc_str, varname, grid_str) for varname in varlist
            ]
            xds = xr.open_mfdataset(filepaths, chunks=chunks, **kwargs)
        xds.attrs["name"] = name
        # load time stamps (like coordinate variables)
        if "time_stamp" in xds:
            xds["time_stamp"].load()
        # fix time axis (deprecated - should not be necessary anymore)
        if lmonthly:
            warn(
                "'lmonthly=True' should only be used to convert simple monthly indices into 'datetime64' coordinates."
            )
        ...
        dataset = DatasetNetCDF(
            name=name, filelist=filepaths, varlist=varlist, multifile=False, **kwargs
        )
        # add GDAL to dataset
        default_geoargs = dict(griddef=grid, gridfolder=grid_folder)
        if geoargs:
            default_geoargs.update(geoargs)
...
def loadSnoDAS(
    ...
    **kwargs,
    ...
    if time_chunks:
        if grid:
            raise NotImplementedError(
                "Default chunks are only available for native lat/lon grid."
            )
        cks = netcdf_settings["chunksizes"] if chunks is None else chunks
        # use default netCDF chunks or user chunks; set time chunking with time_chunks
        chunks = dict(time=time_chunks, lat=cks[1], lon=cks[2])
    ...
    if lxarray:
        ...
        grid_str = "_" + grid if grid else ""
        if bias_correction:
            grid_str = "_" + bias_correction + grid_str
        ...
        # load time stamps (like coordinate variables)
        if "time_stamp" in dataset:
            dataset["time_stamp"].load()
        # add projection
        if lgeoref:
            dataset = checkGeoReference(dataset, geoargs=geoargs, grid=grid)
    else:
...
grid_res = {
    res: 0.00833333333333333 for res in TS_grids
}  # no special name, since there is only one...
...
def loadTimeSeries(lxarray=False, **kwargs):
    return loadSnoDAS_TS(lxarray=lxarray, **kwargs)


def loadClimatology(lxarray=False, **kwargs):
    return loadSnoDAS(lxarray=lxarray, **kwargs)
...
loadShapeClimatology = (
    None  # climatologies without associated grid (e.g. provinces or basins)
)
loadShapeTimeSeries = (
    None  # time-series without associated grid (e.g. provinces or basins)
)
...
if __name__ == "__main__":
    ...
    if lcluster:
        ...
        cluster = LocalCluster(
            processes=True, threads_per_worker=2, n_workers=2, memory_limit=0.3
        )
    ...
    for mode in modes:
        if mode == "load_Climatology":
            ...
            if lxarray:
                print(
                    ("Size in Memory: {:6.1f} MB".format(var.nbytes / 1024.0 / 1024.0))
                )
            ...
            if pntset in ("shpavg", "glbshp"):
                dataset = loadSnoDAS_Shp(shape=pntset, period=(2009, 2018))
            elif pntset in ("oncat"):
                dataset = loadSnoDAS_Shp(shape=pntset, grid=grid, period=(2011, 2019))
            else:
                raise NotImplementedError(pntset)
            print(dataset)
            print("")
            print((dataset.time))
            ...
            print("\n")
            prdstr = "{:04d}-{:04d}".format(
                pd.to_datetime(ts_var.data[0]).year,
            ...
            # length of month
            vatts = dict(
                name="length_of_month", units="days", long_name="Length of Month"
            )
            varnc = add_var(
                ds,
                vatts["name"],
            ...
            xds = loadSnoDAS_TS(
                ...
                lxarray=lxarray,
                geoargs=geoargs,
            )  # 32 time chunks may be possible
            print(xds)
            print("")
            xv = xds[varname]
            ...
            if lxarray:
                print(
                    ("Size in Memory: {:6.1f} MB".format(xv.nbytes / 1024.0 / 1024.0))
                )
            ...
            if lpickle and not lxarray:
                ...
                lon2D, lat2D = np.meshgrid(
                    griddef.xlon.coord, griddef.ylat.coord
                )  # if we have x/y arrays
                griddef.lon2D = lon2D.astype(np.float32)
                griddef.lat2D = lat2D.astype(
                    np.float32
                )  # astype always returns a newly allocated copy

                filename = pickleGridDef(
                    griddef, lfeedback=True, loverwrite=True, lgzip=True
                )
            ...
            for varname in varlist:
                ...
                # slice entire dataset
                xds = xds.sel(time=slice(start_date, end_date))
                # print(xds)
                # print('\n')
                ...
                grid_str = "" if grid is None else "_" + grid
                bc_str = bias_correction + "_" if bias_correction else ""
                tmp_str = tsfile.format(bc_str, varname, grid_str)
                filepath = avgfolder + tmp_str  # native grid...
                # write to NetCDF
                netcdf_encoding = dict(
                    zlib=True, complevel=1, _FillValue=-9999, chunksizes=chunks
                )
                rds.to_netcdf(
                    filepath,
                    mode="w",
                    ...
                    unlimited_dims=["time"],
                    engine="netcdf4",
                    encoding={varname: netcdf_encoding},
                    compute=True,
                )
                ...
                tsnc[:] = np.stack(
                    [str(t) for t in rds["time"].data.astype("datetime64[M]")], axis=0
                )
            ...
            lats = (46, 54)
            lons = (-117, -107)
            multi_chunks = dict(time=32, lat=4, lon=4)

            # testing optionts
            # times = ("2011-01-01", "2011-02-01")
            comp_args = dict(scheduler="threads", num_workers=1)
            varlist = ["snow"]

            # results file
            from geospatial.xarray_tools import saveXArray, purgeAtts

            # file path based on variable parameters
            nc_folder = daily_folder
            nc_filename = f"snodas_{area_name.lower()}_snow_daily.nc"
            print(
                f"Saving area-averaged file '{nc_filename}' to folder:\n '{nc_folder}'"
            )
            ...
            # average over area
            slices = dict(lat=slice(*lats), lon=slice(*lons))
            if times:
                slices["time"] = slice(*times)
            rds = xds.sel(**slices).mean(dim=("lat", "lon"))

            # add attributes
            rds.attrs["resampling"] = "area_average"
            rds.attrs["area_name"] = area_name
            rds.attrs["lat_bnds"] = str(lats)
            rds.attrs["lon_bnds"] = str(lons)
            rds.attrs["start_date"] = str(rds["time_stamp"].values[0])
            rds.attrs["end_date"] = str(rds["time_stamp"].values[-1])
            purgeAtts(rds, atts=["xlon", "ylat", "proj4", "dim_order"])
            print(rds)
            print("")

            # save resampled dataset
            encoding = dict(DEFAULT=dict(zlib=True, complevel=1, shuffle=True, _FillValue=np.NaN),
                            time_stamp=dict(zlib=False, complevel=1, shuffle=False, _FillValue=None))
            saveXArray(
                rds,
            ...
            print("")
            xv = xds[varname]
            xv = xv.loc["2021-01-01":"2021-02-01", 46:54, -117:-107]
            #       xv = xv.loc['2011-01-01',:,:]
            print(xv)
            print(("Size in Memory: {:6.1f} MB".format(xv.nbytes / 1024.0 / 1024.0)))
            ...
            for var in binary_varlist:
                ...
                ds = nc.Dataset(daily_folder + netcdf_filename.format(VAR=var), "a")
            ...
            # optional slicing (time slicing completed below)
            start_date = None
            end_date = None  # auto-detect available data
            ...
            for var in derived_varlist:
                ...
                nc_filepath = "{}/{}".format(
                    folder, netcdf_filename.format(VAR=varname)
                )
                ...
                if not lskip:
                    ...
                    if var == "liqwatflx":
                        ...
                        xvar = xds["snwmlt"].fillna(0) + xds["liqprec"].fillna(
                            0
                        ...
                        ref_var = "liqprec"
                        note = "masked/missing values have been replaced by zero"
                        xvar = xds["liqprec"].fillna(0) + xds["solprec"].fillna(
                            0
                        )  # fill missing values with zero
                        # N.B.: missing values are NaN in xarray; we need to fill with 0, or masked/missing values
                        #       in snowmelt will mask/invalidate valid values in precip
                        ...
                        ref_var = "snow"
                        note = "SWE divided by snow depth, divided by 1000"
                        xvar = xds["snow"] / xds["snowh"]
                    elif varname == "dswe":
                        ref_var = "snow"
                        note = "Rate of Daily SWE Changes"
                        assert xds["snow"].attrs["units"] == "kg/m^2", xds[
                            "snow"
                        ].attrs["units"]
                        xvar = (
                            xds["snow"].diff("time", n=1, label="upper") / 86400
                        )  # per second
                    ...
                    if lappend:
                        ...
                        tasks = xvar.data.map_blocks(
                            save_chunk, chunks=dummy.shape, dtype=dummy.dtype
                        ).compute()
                        ...
                        nds = xr.Dataset(
                            {ts_name: tsvar, var: xvar}, attrs=xds.attrs.copy()
                        )
                        # write to NetCDF
                        tmp_filepath = (
                            nc_filepath + ".tmp"
                        )  # use temporary file during creation
                        var_enc = dict(
                            zlib=True,
                            complevel=1,
                            _FillValue=-9999,
                            chunksizes=netcdf_settings["chunksizes"],
                        )
                        tasks = nds.to_netcdf(
                            tmp_filepath,
                            mode="w",
                    ...
                    if lexec:
                        ...
                        print(
                            f"Writing Dask task visualization file:\n '{task_viz_path}'"
                        )
                        tasks.visualize(
                            filename=task_viz_path
                        )  # This file is never produced and the process appears very slow
            ...
            for varname in ["evap_snow"]:
                ...
                # read data
                data = readBinaryFile(
                    varname=varname, date=date, lgzip=lgzip, lmissing=True
                )
                # some checks
                ...
                print(
                    ("Size in Memory: {:6.1f} MB".format(data.nbytes / 1024.0 / 1024.0))
                )
                # make plot
        #           import pylab as pyl
        #           pyl.imshow(np.flipud(data[:,:])); pyl.colorbar(); pyl.show(block=True)
            ...
            end_date = "2024-01-14"  # non-inclusive end date
            ...
            for varname in ["liqprec"]:
                ...
                if not osp.exists(filepath) or not lappend:
                    ...
                    print(
                        f"\nCreating new NetCDF-4 dataset for variable '{varname}':\n  '{filepath}'"
                    )
                    ncds = creatNetCDF(
                        varname,
                        varatts=netcdf_varatts,
                    ...
                    ncds.sync()
                    ncvar = ncds[varname]
                    win_fp = filepath.replace("/", "\\")
                    win_ncds_fp = ncds.filepath().replace("/", "\\")
                    assert win_fp == win_ncds_fp, (win_fp, win_ncds_fp)
                    ncts = ncds["time_stamp"]
                    nctc = ncds["time"]
                    ...
                    ncds = nc.Dataset(
                        filepath, mode="a", format="NETCDF4", clobber=False
                    )
                    ncvar = ncds[varname]
                    ...
                    time_array = np.arange(
                        restart_date, end_date, dtype="datetime64[D]"
                    )
                    #               time_array = np.arange(start_date,'2009-12-14', dtype='datetime64[D]')
                    if len(time_array) > 0:
                        print(
                            (
                                "First record time-stamp: {:s} (offset={:d}), Time Chunking: {:d}".format(
                                    time_array[0], time_offset, nc_time_chunk
                                )
                            )
                        )
                    else:
                ...
                for i, day in enumerate(time_array):
                    ...
                    if fi == flush_intervall:
                        print(
                            (
                                "Flushing data to disk ({:d}, {:s})".format(
                                    ii, actual_day
                                )
                            )
                        )
