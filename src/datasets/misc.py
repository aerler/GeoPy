...
def detectGridResampling(folder, grid=None, resampling=None):
    ...
    if resampling is None:
        ...
        if nc_file:
            ...
            resampling = default_folder
        elif len(folder_list) == 1:
            resampling = folder_list[0]
    if resampling:
    ...
    **kwargs
    ...
    if dataset is None and varname is not None and mode == "daily" and aggregation == "daily":
        if dataset_index is None:
            raise ArgumentError(
                "A 'dataset_index' dict is required to infer a preferred dataset for a variable."
            )
    ...
    if dataset.lower() == "mergedforcing" or dataset.lower() == "merged":
        ...
        resolution = None
        if grid is None:
            raise ArgumentError("Need to specify 'grid' for MergedForcing dataset!")
    elif dataset.lower() == "nrcan":
        subset = dataset.lower()  # no subsets
        if resolution is None and grid is None:
            raise ArgumentError("Need to specify either 'resolution' or 'grid' for NRCan dataset!")
    elif dataset[:4].lower() == "era5" and len(dataset) > 4:
        if subset is None:
            subset = dataset.lower()
        dataset = "era5"
        if resolution is None and grid is None:
            raise ArgumentError("Need to specify either 'resolution' or 'grid' for ERA5 dataset!")
    else:
        if subset is None:
            subset = dataset.lower()
    ...
    if resolution:
        if grid is None:
            grid = resolution.lower()
            warn("No grid specified, assuming resolution as grid.")
            resolution = None
        elif grid.lower() == resolution.lower():
            resolution = None
            ## N.B.: if resolution and grid are the same, we can omit the resolution in the filename;
            ##       this is convenient, as for most dataset the resolution defines a grid, which can be
            ##       considered the native grid of the dataset
        else:
            ds_str_file += "_" + resolution.lower()
    # check grid
    if grid is None:
        warn("No grid specified, assuming native grid (dataset name).")
        grid = dataset.lower()
    # construct filename
    gridstr = "_" + grid.lower()
    # add shape or station identifier in front of grid
    if shape and station:
        raise DatasetError((shape, station))
    elif shape:
        gridstr = "_" + shape.lower() + gridstr
    elif station:
        gridstr = "_" + station.lower() + gridstr
    bcstr = "_" + bias_correction.lower() if bias_correction else ""
    if aggregation.lower() == "daily":
        name_str = bcstr + "_" + varname.lower() + gridstr
    else:
        name_str = bcstr + gridstr
...
def addConstantFields(xds, const_list=None, grid=None):
    ...
    for rv in xds.data_vars.values():
        if xds.ylat in rv.dims and xds.xlon in rv.dims:
            break
    ...
    llat2D = "lat2D" in const_list
    llon2D = "lon2D" in const_list
    if llat2D or llon2D:
        ...
        if grid is None:
            ...
            for dim in xds.dims:
                if dim in default_lon_coords:
                    xlon = xds[dim].values
                if dim in default_lat_coords:
                    xlat = xds[dim].values
            if xlon is None or xlat is None:
                raise DatasetError(
                    "Need latitude and longitude coordinates or GridDef object to infer lat2D or lon2D."
                )
...
def loadXRDataset(
    ...
    **kwargs
    ...
    folder, filename = getFolderFileName(
        varname="{var:s}",
        dataset=dataset,
        subset=subset,
    ...
    if resolution is not None and "resolution" not in xds.attrs:
        xds.attrs["resolution"] = resolution
    if resampling is not None and "resampling" not in xds.attrs:
        xds.attrs["resampling"] = resampling
    if "name" not in xds.attrs:
